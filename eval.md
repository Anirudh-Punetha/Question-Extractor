...
* **Dependencies:** `jq` (Command-line JSON processor) must be installed to run the evaluation script.
...
```bash
bash run_eval.sh ./input_pdfs ./output_results
```
...
A. Performance & Stability (Key Constraints - 30 pts)
...
B. Structural Fidelity (Correctness - 25 pts)
...
C. Hierarchical Integrity (Schema - 20 pts)
...
Time-Boxing: Each segment is assigned a 150-second timeout. If a chunk hangs, the system kills that specific task and logs a partial failure.
...
When run_eval.sh finishes, the output_results folder will contain:File TypePurposefilename.jsonThe final structured output. Includes questions, errors, and processing_time_sec.storage/assets/The repository for all extracted PNGs and CSVs, uniquely named per Job ID.Console OutputReal-time status logs showing [✓] Done in Xs. Warnings like ! 1 chunks failed indicate partial successes where the timeout was triggered.
